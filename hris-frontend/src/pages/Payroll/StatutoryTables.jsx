...
function StatutoryTables() {
    ...
    const submit = async () => {
        ...
        if (!form.effective_from) return toast.error('Effective from date is required.');
    ...
    return (
        <div className="mx-auto max-w-7xl space-y-6">
            <div className="border-b border-slate-100 pb-4">
                <CustomLabel variant="h2" addedClass="font-bold text-slate-700!" description="SSS / PhilHealth / Pag-IBIG / withholding-tax rules that drive the payroll engine.">
                    Statutory Tables
                </CustomLabel>
            </div>
            ...
            <CustomDataTable
                ...
                actionButton={can('statutory-and-compliance:create') && (
                    <CustomButton onClick={() => setForm({ ...BLANK })} icon={PlusIcon} iconPosition="left"
                        className="flex items-center gap-2 bg-slate-800 px-4 py-2 text-sm hover:bg-slate-700">Add Table</CustomButton>
                )}
            />
                ...
                footer={(
                    <div className="flex justify-center border-t border-slate-100 pt-4">
                        <CustomButton onClick={submit} icon={Save} iconPosition="left" isLoading={isMutating}
                            disabled={isMutating || !form?.label || !form?.effective_from}
                ...
                {form && (
                    <div className="max-h-[65vh] space-y-4 overflow-y-auto px-1">
                        ...
                        <div>
                            ...
                            <p className="mt-1 text-xs text-slate-400">{COMPUTATION_HELP[method]}</p>
                        </div>
